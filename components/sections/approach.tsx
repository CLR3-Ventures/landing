...
export function Approach() {
  return (
    <section id="approach" className="scroll-mt-20 border-t border-line py-24 sm:py-32">
      <div className="container-x">
        ...
        <ol className="mt-14 grid gap-px overflow-hidden rounded-lg border border-line bg-line md:grid-cols-2 lg:grid-cols-4">
          {approach.map((a, i) => (
            <li key={a.step} className="relative bg-background p-7 sm:p-8">
              <div className="flex items-center gap-3">
                <span className="inline-flex size-7 items-center justify-center rounded-full border border-signal/40 bg-signal-soft text-xs font-semibold text-signal">
                  {i + 1}
                </span>
                <span className="text-sm font-medium text-ink-faint">{a.step}</span>
              </div>
              <h3 className="mt-5 text-balance text-lg font-semibold leading-snug tracking-[-0.015em] text-foreground">
                {a.title}
              </h3>
